...
(function () {
  ...
  function initArticleScroll() {
    ...
    function onScroll() {
      ...
      var line = window.scrollY + 140, found = -1;
      for (var i = 0; i < targets.length; i++) {
        if (targets[i] && targets[i].offsetTop <= line) found = i;
      }
      if (found !== current) {
        if (links[current]) links[current].classList.remove('active');
        if (links[found]) {
          links[found].classList.add('active');
          // keep the active item in view inside a scrollable TOC
          var ol = links[found].closest('ol');
          if (ol && ol.scrollHeight > ol.clientHeight + 8) {
            var r = links[found].getBoundingClientRect(), rr = ol.getBoundingClientRect();
            if (r.top < rr.top + 20 || r.bottom > rr.bottom - 20) {
              ol.scrollTop += (r.top - rr.top) - ol.clientHeight / 2 + r.height / 2;
            }
          }
        }
        current = found;
  ...
  function initFloatingCta() {
    ...
    if (!onContact) {
      ...
      a.innerHTML = '<span class="fc-dot"></span> Book 30-Min Tech Session <span class="fc-arrow">\u2192</span>';
